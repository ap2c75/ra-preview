...
const aiCatalog=respond(initialState(),{text:'물 먹는 기계 싼 거 셋 골라줘',ai:{route:'catalog',normalizedText:'저렴한 정수기 3개 추천해줘',confidence:0.9}},catalog,context);
assert.equal(aiCatalog.cards.length,3);
assert.ok(aiCatalog.cards.every(card=>/정수기|이온수기/.test(card.name)));
